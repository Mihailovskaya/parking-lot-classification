...
from src.utils.metrics import fitness, fitness_p, fitness_r, fitness_ap50, fitness_ap, fitness_f   
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Filter by class
        if classes:
            x = x[(x[:, 5:6] == torch.tensor(classes, device=x._device)).any(1)]
...
def strip_optimizer(f='weights/best.pt', s=''):  # from utils.general import *; strip_optimizer()
    ...
    x = torch.load(f, map_location=torch._device('cpu'))
    x['optimizer'] = None
...
def apply_classifier(x, model, img, im0):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            pred_cls2 = model(torch.Tensor(ims).to(d._device)).argmax(1)  # classifier prediction
